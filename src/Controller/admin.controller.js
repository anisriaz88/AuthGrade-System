...
// create a admin account (only for testing, should be removed in production)
export const createAdmin = asyncHandler(async (req, res) => {
  ...
  res.status(201).json(
    new ApiResponse(201, "Admin user created successfully", {
      user: {
        _id: user._id,
        name: user.name,
        email: user.email,
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  if (updates.role) {
    throw new ApiError(
      400,
      "You cannot update role through this endpoint",
    );
  }
